    ...
    fetch('/api/items')
        ...
        .then(data => {
            if (data.items && data.items.length > 0) {
                ...
                // 첫 36개 표시
                const maxItems = Math.min(data.items.length, 36);
...
function createAndAppendGridItem(item, grid) {
    ...
    // 다중 이미지가 있는 경우 2번째 이미지 요소 추가
    if (hasMultipleImages && secondImageSrc) {
        // 첫 번째 이미지에 클래스 추가
        ...
        secondImg.classList.add('second_image');
        
        // 이미지 순서대로 추가
        link.appendChild(img);
        link.appendChild(secondImg);
    } else {
        link.appendChild(img);
    }
...
function loadMoreItems() {
    ...
    const nextBatch = allItems.slice(currentOffset, currentOffset + 48);
...
function createGridItem(item) {
    ...
    if (hasMultipleImages) {
        ...
        img.dataset.hoverSrc = secondImageSrc;
    } else {
        gridItem.classList.add('single_image');
    }
